...
2. **Build the migration scope** (one row per Key Vault, joined to its Function App/subnet/existing PE):
   ...
   ```
   This is the exact scope list used for CAB submissions and as the `-BatchInventory` input to the scripts in `scripts/migration/`. A Function App identity is matched via **either** an RBAC role assignment **or** a classic Key Vault Access Policy entry that actually grants `get` on secrets (a vault can use either authorization model) — the `AccessMechanism` column in the output CSV shows which one resolved the match, and `CandidateFunctionApps` always lists every matching identity (not just the one used). Rows flagged `ReviewRequired = true` — no match via either mechanism, multiple matching identities that sit in **different** subnets (ambiguous: which subnet gets the Service Endpoint/VNet rule), or an Access Policy entry found for the identity but missing `get`/`all` on secrets (`AccessPolicyInsufficientPermissions = true`) — must be resolved manually before being included in any batch; never guess the mapping. Multiple identities that happen to share the same subnet are **not** flagged - any of them is an equally correct answer for migration-scoping purposes.
